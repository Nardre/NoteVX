...
void sha256_chain(const BYTE *input, size_t input_len, unsigned int rounds, BYTE hash[SHA256_BLOCK_SIZE])
{
    SHA256_CTX ctx;
...
int gethostsubname(char hostname[HOST_NAME_MAX]) {
    ...
    memmove(hostname, subname, strlen(subname) + 1);
    return 0;
}
...
int protection(void) {
    ...
    unsigned int rounds = 0x1444;
    const BYTE expected[SHA256_BLOCK_SIZE] = {
        0x52, 0x5e, 0xc4, 0xfa, 0xad, 0xbb, 0xea, 0x5f,
        0x78, 0x6d, 0xcc, 0xa6, 0x76, 0x51, 0x12, 0x41,
        0x96, 0xba, 0xa2, 0x0f, 0xf5, 0x39, 0x74, 0x69,
